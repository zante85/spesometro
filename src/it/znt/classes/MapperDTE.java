...
public class MapperDTE {
	...
	public Document createCedentePrestatoreDTE(Element dteElement, Document doc, Soggetto soggetto) {
		Element rootElement = doc.createElement("CedentePrestatoreDTE");
		dteElement.appendChild(rootElement);
	...
	public Document createCessionarioCommittenteDTE(Element dtrElement, Document doc, Soggetto soggetto,
			List<Fattura> fatture) {
		Element rootElement = doc.createElement("CessionarioCommittenteDTE");
		...
		if (soggetto.getPiva() != null && !soggetto.getPiva().startsWith("9")
			...
			if (!GeneralMethods.isNumeric(soggetto.getPiva())) {
				String[] nomeCognome = soggetto.getDenominazione().split(" ");
				soggetto.setDenominazione(null);
				if (nomeCognome != null && nomeCognome.length >= 2) {
					soggetto.setCognome(nomeCognome[0]);
					soggetto.setNome(nomeCognome[1]);
					soggetto.setDenominazione(null);
				}
			}
			Element codiceFiscale = doc.createElement("CodiceFiscale");
		...
		comune.appendChild(doc.createTextNode(soggetto.getSede().getComune()));
		/*************************************************************/
		if (soggetto.getDenominazione() == null || !soggetto.getDenominazione().startsWith(Constant.SPECIAL_CHAR)) {
			Element provincia = doc.createElement("Provincia");
			sede.appendChild(provincia);
			provincia.appendChild(doc.createTextNode(soggetto.getSede().getProvincia()));
		}
		...
		for (Fattura fattura : fatture) {
			doc = createDatiFatturaDTE(rootElement, doc, fattura);
		}

		return doc;
	}

	private Document createDatiFatturaDTE(Element cedentePrestatoreDTE, Document doc, Fattura fattura) {
		Element rootElement = doc.createElement("DatiFatturaBodyDTE");
		cedentePrestatoreDTE.appendChild(rootElement);
		/*************************************************************/
		...
		// natura.appendChild(doc.createTextNode(fattura.getNatura()));
		/*************************************************************/
	...
	private void addDatiRiepilogo(Element rootElement, Document doc, double imponibileFatt, double impostaFatt,
		...
		System.out.println("Imponibile:" + imponibileFatt);
